class FriendlistsController < ApplicationController
  ...
  def block_friend
    # @friend = User.find(params[:id])
    redirect_to request.referer if current_user.block_friend(@friend)
    flash[:alert] = "Blocked @#{@friend.username}."
  end

  def unblock_friend
    # @friend = User.find(params[:id])
    redirect_to request.referer if current_user.unblock_friend(@friend)
    flash[:notice] = "Unblocked @#{@friend.username}."
  end

  def delete_friend
    # @friend = User.find(params[:id])
    redirect_to request.referer if current_user.remove_friend(@friend)
    flash[:alert] = "Deleted @#{@friend.username} from friendlist."
  end

  def accept_friend
    # @friend = User.find(params[:id])
    redirect_to request.referer if current_user.accept_request(@friend)
    flash[:success] = "Accepted @#{@friend.username}'s request."
  end

  def decline_friend
    # @friend = User.find(params[:id])
    redirect_to request.referer if current_user.decline_request(@friend)
